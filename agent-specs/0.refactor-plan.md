...
| 22  | **MVP-22** | 可规划景点门槛 + 餐档骨架 + 填站搜餐；**景点库最后**（F84–F87 / ADR-049） | **下一批主线；S1 先做** |

**已关闭：** 原 `TBD-1.md`（Trip Store / 按日并发）→ 决议写入 [ADR-046](../../workspace-specs/adr/ADR-046-trip-store-pg-memory-fetch.md)；按日并发**默认不切**；实现见下方批次 16。

### 剩余待办与开工队列（2026-09-04）

一次一条故事。**当前主线 = 批次 22。** 下表是仍开项与 ADR-049 的兼容，不是新开一批平行需求。

| 仍开项 | 原批次 | 与 22 的关系 | 处理 |
| --- | --- | --- | --- |
| F84–F87 | **22** | 主线 | **按 S1→S2→S3→S4**；S4 最低优先 |
| F41 Story 4（make / fetch / fill） | 20 | 消费 F84 后的干净池；餐档 UI 跟 F85 | **F84 DoD 后再做**；禁 stay-only 已有 F80/F83，S4 只做 2play 走通 |
| F41 Story 3（步骤 g 才 discover） | 20 | 与已落地 S2「静默 discover」重叠 | **默认不做**；若仍要「g 才搜」另开故事，不得打回脏芯片 |
| F41 Story 1–2 usable 签收 | 20 | 不挡 F84 | 主线走通后再签；不重做 intake |
| 2play F40 P3 / 里斯本 3 日浏览器 | 19 | 杭州合称 502 比里斯本酒店-only 更挡 make | **F84（+ 建议 F85）之后**再签收；夹具含杭州 + 里斯本 |
| 批次 18 P2：68 / 70 / 73 / 37f | 18 | 皮肤/文案/测对齐 | **停放**；不插入 22 中间 |
| MVP-19 DoD「无 POI 新表」 | 19 | 与 F87 字面冲突 | **改为：无源码城表**（ADR-042）。跨行程运行时库 = F87，最后做 |
| F57 区域 must_include 展开 | 13 ✅ | 名胜区被展开会再脏池 | F84 谓词优先：合称/名胜区 **不展开成景点**；不对 F57 开新故事除非回归 |
| F80 必去=stop 名 / 禁 stay-only | 19 ✅ | 合称当 must_include 仍会 502 | F84 **先降级**再跑 F80；不放宽 stay-only |
| F78 超时 / F81 fetch 同流 | 19 ✅ | `patchTrip` 升 revision | 写后仍 fetch；**不**上推送；不把 patch 暴露给 BFF |
| F79 先池后热度 | 19 ✅ | 芯片应来自 eligible 子集 | F84 过滤后再打热度；不恢复 ungrounded 打标 |
| F39 / F41（MVP-9 tsc / opt-in） | 10 | 与规划无关 | **停放**；不挡 22 |
| 硬删 `arrange_day` | 12 | gate 2play | 仍不新开 `plan_day_trip`（ADR-049） |

**推荐开发批次（严格顺序）：**

```text
22-S1  F84  eligible + 内部 patchTrip + must_include 降级     ← 现在做
22-S2  F85  骨架餐档（无餐馆名）
20-S4  2play F41 Story 4  make→fetch→fill（干净池 + 餐档展示）
22-S3  F86  plan_next_stop 按邻站搜餐
签收   杭州 3 日 make 不 502 + 里斯本 3 日非 stay-only（原 19-P3）
停放   18-P2 体验、F41-S3、MVP-9 债
22-S4  F87  目的地景点库（最后；不提前）
```

**依赖：** 22-S1 不依赖 20-S3/S4。20-S4 依赖 22-S1（建议也等 22-S2，否则 UI 仍可能展示骨架餐馆名）。22-S3 依赖 22-S2。22-S4 只依赖 22-S1 谓词，但排期在最后。
...
**真源：** 本文件本节 · [ADR-045](../../workspace-specs/adr/ADR-045-iconic-places-unified-acquisition.md) · [ADR-042](../../workspace-specs/adr/ADR-042-no-city-encyclopedia-in-source.md) · Feature **37** `plan-46` · 2026-09-02 里斯本规划走查（invalid_input / chip / 必去地 / 出行贴士）。
...
**真源：** 本文件本节 · [ADR-046](../../workspace-specs/adr/ADR-046-trip-store-pg-memory-fetch.md) D3/D5/D7 · [ADR-045](../../workspace-specs/adr/ADR-045-iconic-places-unified-acquisition.md) · [ADR-042](../../workspace-specs/adr/ADR-042-no-city-encyclopedia-in-source.md) · `[agent-design.md](./agent-design.md)` §20.11 / §21.6 / §22 · `[agent-stories.md](./agent-stories.md)` F74–F77 · 2play Feature **37** AC8c / AC13c / AC24–AC27 · 2026-09-02 设计评审（采纳：懒创建 `trip_id`；`findIconicPlaces` 不升 HTTP；`plan_next_stop` 非按日工具；tips 超时仍落 iconic）。
...
**真源：** [`e2e-test-results/reproduce.md`](./e2e-test-results/reproduce.md) · `[agent-design.md](./agent-design.md)` §20.4 / §24 · `[agent-stories.md](./agent-stories.md)` F78–F82 · 2play `[2play-design.md](../../3.where2play/2play-specs/2play-design.md)` §4.10–§4.11 · Feature **40** · [ADR-042](../../workspace-specs/adr/ADR-042-no-city-encyclopedia-in-source.md) / [ADR-045](../../workspace-specs/adr/ADR-045-iconic-places-unified-acquisition.md) 补丁评估 / [ADR-046](../../workspace-specs/adr/ADR-046-trip-store-pg-memory-fetch.md)
...
- [ ] 实现按上表一次一条
- [ ] 里斯本 3 日：骨架每日非 stay≥1；make 502 可 fetch 恢复或明确失败
...
**2play Feature 41** `plan-49` · [`2play-stories.md`](../../3.where2play/2play-specs/2play-stories.md) · [`2play-design.md`](../../3.where2play/2play-specs/2play-design.md) §4.12
...
| **S4** | F41 Story 4 | make / fetch 骨架 / fill；禁 stay-only 当成功 | **ToDo · 接 22-S1**（建议接 22-S2）；agent 硬闸已在 F80/F83 |
...
**规格：** 2play Feature **41 Story 5** · agent Feature **83** · [ADR-048](../../workspace-specs/adr/ADR-048-skeleton-geo-anchor-is-destination.md)
...
**日期：** 2026-09-04  
**原因：** 杭州 3 日池不空仍 `make_itinerary` 502：合称/名胜区进芯片与 `must_include`；骨架绑餐馆名。目的地景点库只加速复用，不止血。决议：[ADR-049](../../workspace-specs/adr/ADR-049-verified-attraction-and-meal-slots.md)。
...
| **S2** | **F85** `skeleton-meal-slots` | `make_itinerary` 只排 eligible 景点顺序 + 每日午餐/晚餐 **slot**（无店名）。改骨架走 `patchTrip`。校验不再要求骨架覆盖餐馆名 | S1 | ToDo |
| **S3** | **F86** `fill-resolve-meals` | `plan_next_stop` 在餐档窗口按相邻景点搜餐馆并落点；失败则空档可跳过、不整日失败。落点/跳过走 `patchTrip`。缺餐馆详情 **不**当景点池脏数据 | S2 | ToDo |
| **S4** | **F87** `destination-poi-registry` | PG Destination + AttractionPoi（`place_id` / `native_id` / 别名）；discover upsert **仅 eligible**；make 可先读库再补搜。L1 详情异步。**最低优先级** | S1（谓词）；建议 S2 已稳 | ToDo（最后） |
...
**开工顺序：** 总览「推荐开发批次」。本批内：**只做 S1** 至 DoD → S2 →（插入 2play F41 S4）→ S3 → 签收 → **最后 S4**。不得为 F87 推迟 S1。不得在 S1 前插入 18-P2 / F41-S3 / 里斯本签收。
...
- [ ] F85：骨架无餐馆店名；餐档可 fetch
- [ ] F86：填站可落餐或明确跳过
- [ ] F87：非源码表；upsert 走 F84 谓词；非目录城（如 Lisbon）路径与杭州相同
