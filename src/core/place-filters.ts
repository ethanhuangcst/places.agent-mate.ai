...
const ATTRACTION_ALLOW =
  /museum|park|landmark|tourist_attraction|monument|gallery|temple|church|castle|viewpoint|miradouro|zoo|aquarium|palace|bridge|memorial|scenic|景点|博物馆|博物館|公园|公園|风景|風景|名胜|名勝|古迹|古跡|寺庙|寺廟|园林|園林|展览|展覽|美术馆|美術館|history_museum|botanical|archaeolog|cathedral|科教文化|风景名胜|風景名勝|文物古迹|文物古蹟|纪念馆|紀念館|展览馆|展覽館|观光|觀光|人文景观|人文景觀|14\d{4}/i;
...
const LANDMARK_AS_MEAL_DENY =
  /广州塔|东方明珠|上海中心|管理办|码头广场|贵宾楼|怡宾楼|迎宾楼|步行街综合|琶醍|^宾馆|酒店楼/i;

function blobOf(place: PlaceCard): string {
  return `${place.name} ${place.category ?? ""}`;
}

/** Timed visits: attractions only — never lodging, malls, plazas, stations, or docks. */
export function filterAttractionPlaces(places: PlaceCard[]): PlaceCard[] {
  return places.filter((p) => {
    const blob = blobOf(p);
    if (LODGING_DENY.test(blob) || VISIT_DENY.test(blob)) return false;
