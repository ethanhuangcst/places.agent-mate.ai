...
**已知限制（非本批阻塞）：** `host_instructions` 无法强制宿主 LLM 工具调用纪律（并发/问确认），措辞 step2 为最不坏版本；2play Mode H 调用侧仍有 origin/dest 传 name-only、解析 schema 删 from_origin/to_destination 等待修项（见 2play 侧 review）。
