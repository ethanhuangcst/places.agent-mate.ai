...
| Admin UI | [`3.ui-design.md`](./3.ui-design.md) |
| Admin UI mock-ups | [`ui-mockup/`](./ui-mockup/) |
| Test strategy | [`4.test-strategy.md`](./4.test-strategy.md) |
| Technical design | [`5.agent-design.md`](./5.agent-design.md) |
...
## MVP plan (three slices, by agent capability)
...
| **Enrich Tripadvisor** | Optional ratings/content by name + location | **8** | **MVP-3** |
| **Place chat** | NL tool loop on Quanzil over the tools already shipped | **10** | **MVP-3** |
...
| **MVP-2 — Search places + plan itinerary** | where2play can search POIs and request a structured itinerary (weather keys in Feature 13). | **2, 9** |
| **MVP-3 — Enrich + place chat** | Tripadvisor match on cards; ChatBox NL chat reuses the tool core. | **8, 10** |
...
- NL chat is a Quanzil loop over tools from MVP-1/2. Do not invent a second tool core.
- Tripadvisor enrich is optional on search/details; never pass Google `place_id` as a Tripadvisor id.

**Build order inside a slice:** one user story to DoD at a time ([`5.agent-design.md`](./5.agent-design.md) §16). Suggested MVP-1 order: **14 → 15 → 16 → 19 → 18 → 17 → 12 → 11 → 6 → 5 → 1 → 3 → 7 → 4 → 13**.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | agent | Restaurant search | `places-agent-search-restaurants` | Search restaurants by location and criteria via the caller’s requested providers | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 2 | agent | Place search | `places-agent-search-places` | Search non-restaurant places (attractions, POIs) the same way | [2.agent-ac.md](./2.agent-ac.md) | **2** |
| 3 | agent | Place details | `places-agent-place-details` | Fetch details for a known place using a provider-native id | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 4 | agent | Navigation helpers | `places-agent-navigate` | Return secret-free navigation deep links and URLs for a place | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 5 | agent | Geocoding | `places-agent-geocode` | Geocode and reverse-geocode as needed so search can run from an address or a pin | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 6 | agent | Map vendor selection | `places-agent-map-vendors` | Callers pass which **map vendors** to query (`providers[]`); agent validates credentials and capabilities; no silent vendor swap | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 7 | agent | Place-card sources | `places-agent-card-sources` | Every place card lists `sources[]`; optional merge of duplicates; the **app** chooses which map deep link to open | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 8 | agent | Tripadvisor enrichment | `places-agent-tripadvisor-enrich` | Optional Tripadvisor ratings/content by name + location match; never pass Google `place_id` as an id | [2.agent-ac.md](./2.agent-ac.md) | **3** |
| 9 | agent | Itinerary planning | `places-agent-plan-itinerary` | Structured itinerary suggestions from trip bounds, place results, and traveler preferences (including natural-language); trip UX stays on the caller | [2.agent-ac.md](./2.agent-ac.md) | **2** |
| 10 | agent | Natural-language place chat | `places-agent-nl-chat` | Traveler asks in natural language (optional file/image upload); agent runs a tool loop on server Quanzil | [2.agent-ac.md](./2.agent-ac.md) | **3** |
| 11 | agent | HTTP API and MCP | `places-agent-http-mcp` | Same tools over the HTTP API (app BFFs) and MCP (agent hosts); both identify the service as `places-agent` | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 12 | agent | Caller API key auth | `places-agent-caller-trust` | Serve HTTP/MCP only to callers authenticated with a **caller API key**; map-vendor keys stay on the agent; user-visible errors as i18n keys | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 13 | agent | Bilingual output | `places-agent-bilingual-output` | Agent user-visible output in `EN` / `CN` / `HK` / `TW`; one locale or a bilingual pair. Open-Meteo `weather.wmo.*` keys land with Feature 9 | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 14 | app | Admin home | `places-agent-admin-home` | Public home: link to agent instructions (Feature 18) and an admin login control | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 15 | app | Admin login and users | `places-agent-admin-users` | Admin sign-in, default admin, password reset and invites via Resend; public register disabled | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 16 | app | Admin landing | `places-agent-admin-landing` | After login: left navigator; header with agent instruction link and greeting with the signed-in user name | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 17 | app | Caller API keys | `places-agent-admin-api-keys` | Create, edit, regenerate, delete caller API keys; copy secret | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 18 | app | Agent instruction | `places-agent-admin-instructions` | How to call places.agent-mate.ai; two entries — public home link and post-login header link | [2.agent-ac.md](./2.agent-ac.md) | **1** |
| 19 | app | Admin app i18n | `places-agent-admin-i18n` | Management UI and emails in `EN` / `CN` / `HK` / `TW`; locale switch; missing-key fallback | [2.agent-ac.md](./2.agent-ac.md) | **1** |
