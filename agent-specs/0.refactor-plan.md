...
| 16  | **MVP-16** | 架构优化：Trip Store（PG+内存）+ `fetch_trip_details` + 删 display + 工具精简（F63–66）+ **where2play 消费端对齐** | 🟢 P0 Done（F63/64 + F66 评估）；P1 F65 / 2play 待办 |
...
| `travel_tips` / `visa_requirement` | **保留** | artifacts 挂接延后 P2 |
