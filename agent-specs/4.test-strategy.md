...
- Google transport (ADR-017): fixture **direct success** (Worker MCP not called); fixture **direct egress fail → Worker MCP success** with provenance still `GOOGLE_MAPS` (never `GMAPS_MCP` in `sources[]`); fixture **direct fail + MCP unconfigured** → skip, no AMAP fill.
