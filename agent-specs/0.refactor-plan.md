...
| 6   | **MVP-5**  | Admin 加固 + Error Boundary + 密码重置 E2E                        | ✅ 基本完成；缺密码重置 E2E              |
| 7   | **MVP-6**  | Prompt 组装器 + LLM 行程规划 + MCP 工具拆分 + Token 优化 + 配图           | ✅ DoD 通过 (2026-08-21) ADR-032 |
| 8   | **MVP-7**  | 测试收尾 + 部署准备                                                 | **下一个**                       |
...
| Feature flag `ITINERARY_MODE`；Plan 写默认 `**llm**`              | ⚠ 代码 `?? "legacy"`；`.env.local` 未设 → **实际跑 legacy**                  |
| 候选限 top **15**；LLM 超时 45s                                     | ⚠ 续作改为 top **8**、max_tokens 2048（token 优化）                           |
| user message 含距离矩阵                                            | ⚠ 现行只传 lat/lng，由 LLM 自判（§9.2）                                        |
| MCP `discover_places` / `arrange_day`                         | ✅ MCP 已注册（**Claude Plan 原文未写**；后续提交追加）                               |
| HTTP `/v1/discover_places`、`/v1/arrange_day`                  | ❌ 仅 MCP；design 标了 ✅ 需改诚实                                             |
| Specs：design §9、stories Feature 24/25、test-plan TC-M6-*（约 43） | 部分：design §9 ✅；stories ❌；test-plan 有「MVP-6 续」≠ Plan 全表               |
...
| `agent-design.md` §9.2 | 单 LLM + Zod + 边界   | ✅ 已写；HTTP/默认值与代码不一致                          |
| `agent-stories.md`     | Feature 24 + 25    | ❌ **未补**                                     |
...
1. `**ITINERARY_MODE` 默认值**：文档意图 `llm`；实现仍 `?? "legacy"`（须显式 env 或改代码）。
2. **HTTP** `discover_places` / `arrange_day`（TC-M6-DP05 / AD08 pending）。
3. **password-reset E2E**（TC-M5-E2E02 pending）。
4. Claude Plan 大量 E2E-live 边界用例 — 按需脚本化或正式降级为 backlog。
5. **决策：** `enrich_place` MCP（早期草稿）取消或排期；Guide 文案是否列出新 MCP 名。



### MVP-7 建议顺序（文档债已清）

1. 代码：`ITINERARY_MODE` 默认对齐 **或** 保持 legacy 并在运维文档强调必配。
2. 可选：HTTP discover/arrange；password-reset E2E。
3. caller E2E / live perf；`make quality`；Branches ≥80%。
4. Guide / 部署清单扫尾。
...
| C E2E      | password-reset + invite                        | 邀请 ✅；**password-reset ❌** |
...
- 缺口：Plan 中大量 E2E-live/边界；HTTP DP05/AD08

---



## 批次 8: MVP-7 — 收尾 + 部署准备 **下一个**


| 操作  | 范围                                      | 备注                              |
| --- | --------------------------------------- | ------------------------------- |
| 更新  | `agent-stories.md`                      | Claude Plan F24/F25 + MVP-3b～5  |
| 更新  | `agent-design.md`                       | 默认值/HTTP/overlays 列表诚实化；§5.3 冲突 |
| 更新  | `agent-test-plan.md`                    | TC-M5；与 Claude 矩阵对齐或显式缩减        |
| 补   | password-reset E2E                      |                                 |
| 新增  | README.md                               | 架构 + MCP 工具 + 部署                |
| 跑   | caller E2E / live perf / `make quality` |                                 |
| 决策  | enrich_place MCP；未做 TC 废弃或补做            |                                 |
...
- [ ] README + stories/design/test-plan 与实现一致
- [ ] `ITINERARY_MODE` 默认行为与文档一致
