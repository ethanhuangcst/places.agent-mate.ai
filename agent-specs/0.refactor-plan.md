...
| 23  | **MVP-23** | 规划行程细节：fill + 交通闸 + 顺路餐 + S4–S6 + **S7 起点确认** + **S8 一日游排餐**（F41-S4 · F88–F92） | **S1–S8 Done** |
...
一次一条故事。**当前主线 = 批次 23。** 下表与 ADR-049 / 已落 22 兼容，不平行新开百科或 `plan_day_trip`。

| 仍开项 | 原批次 | 与 23 的关系 | 处理 |
| --- | --- | --- | --- |
| F41 Story 4 fill + F88–F92 | **23** | 主线 | **S1–S5 Done** |
| F84–F87 | **22** | 已实现切片 | 不重做门槛/餐档骨架/库；F86 邻站搜餐被 23-S3 **加严**（池无餐馆） |
| F41 Story 4 骨架 | 20 / 签收 | 23 前置 | 杭州+里斯本 3 日骨架签收 Done；fill **不得**在无骨架时开工 |
| F41 Story 3（步骤 g 才 discover） | 20 | 无 | **默认不做** |
| F41 Story 1–2 usable | 20 | 不挡 23 | 主线走通后再签 |
| 2play F40 P3 骨架签收 | 19 | 已完成探针 | 不挡 23；18-P2 体验仍停放 |
| 批次 18 P2：68 / 70 / 73 / 37f | 18 | 皮肤 | **停放**；不插入 23 中间 |
...
| F39 / F41（MVP-9 tsc / opt-in） | 10 | 与规划无关 | **停放**；不挡 23 |
| 硬删 `arrange_day` | 12 | gate 2play | 仍不新开 `plan_day_trip`（ADR-049） |

**推荐开发批次（严格顺序）：**

```text
22      F84–F87 + 杭州/里斯本骨架签收     ← 已落（usable 可后补）
23-S1   2play F41-S4 fill 接线 + 助手/主区逐站（去「骨架预览」）  ← Done
23-S2   F88  交通闸（preference dual + drop + clock max + dual UI）← Done；打卡串停留 leftover
23-S3   F89  顺路现搜餐 / 挪档 / 插餐 + 内部 patchTrip  ← Done
23-S4   F91 + F90-1  骨架 native_id 指针算路 + 新餐窗（禁止 skip）+ 停留表 + 单景点拆上下午 + 审天不砍景点  ← Done
23-S5   F92  起点 stay 坐标 + UI「起点」；搜餐圆心=当天景点池坐标；主题日保留餐位；早到钉窗不挪景点  ← Done
23-S6A  起点不在目的地附近则拦住（无坐标/远地/search 失败 → 422；文案插 destination）  ← Done
23-S6B  一日游餐贴景点 5km（800m→2km→5km；禁 80km 放行）  ← Done
23-S7   起点确认：店名对得上才自动通过；否则 A/B/C 候选；空发送=目的地坐标  ← Done
23-S8   单景点 AM→lunch→PM；F61 不插午餐到景点前；午餐硬 5km；晚餐可用酒店附近  ← Done
停放    18-P2 体验、F41-S3、MVP-9 债
```

**依赖：** 23 依赖 22 门槛 + 餐档骨架 + 骨架非 stay-only。23 **不**依赖 F87 库 usable。F87 不插入 23 中间。不扩 CATALOG。HTTP `patch_trip` 仍只改 constraints。
...
**规格：** [agent-design §25](./agent-design.md) · [2play-design §4.11](../../3.where2play/2play-specs/2play-design.md) · 本表。一次一条故事。
