...
**Selectors (`data-testid`):** `admin-home-instructions`, `admin-login`, `register-disabled`, `login-submit`, `login-error`, `accept-invite-submit`, `accept-invite-done`, `accept-invite-sign-in`, `accept-invite-error`, `landing-instructions`, `admin-hello`, `nav-keys`, `nav-users`, `nav-sign-out`, `issue-key`, `keys-table`, `keys-empty`, `copy-secret`, `users-table`, `delete-admin-confirm`, `locale-EN` … `locale-TW`. Per-row delete: `delete-admin-{id}`.
