...
describe("validateSkeleton", () => {
  ...
  it("should_drop_attraction_when_name_matches_stay_hotel", () => {
    const hotel = "香港海洋公园万豪酒店";
    const hotelTrad = "香港海洋公園萬豪酒店";
    const raw = {
      days: [
        {
          day_index: 1,
          day_theme: "Ocean Park day",
          stops: [
            { name: hotelTrad, kind: "stay" },
            { name: hotelTrad, kind: "attraction" },
    ...
    const trimmed = dropUnknownAttractionStops(raw, {
      places: [place("Torre de Belém")],
      restaurants: [],
      stays: [hotel],
    });
    const days = (trimmed as { days: Array<{ stops: Array<{ name?: string; kind: string }> }> }).days;
    const attractions = days[0]!.stops.filter((s) => s.kind === "attraction");
    expect(attractions.every((s) => s.name !== hotel && s.name !== hotelTrad)).toBe(true);
    expect(attractions.some((s) => s.name === "Torre de Belém")).toBe(true);
    expect(days[0]!.stops.some((s) => s.kind === "stay")).toBe(true);
  });

  it("should_accept_stay_when_name_is_cjk_variant_of_origin", () => {
    const hotel = "香港海洋公园万豪酒店";
    const hotelTrad = "香港海洋公園萬豪酒店";
    const raw = {
      days: [
...
describe("makeItinerary events (TC-M10-43-01)", () => {
  ...
  it("TC-M19-82-02 should_mark_user_requested_not_must_see_on_supplementary_backfill", async () => {
    ...
    // ADR-069: supplementary backfill is user_requested, not must_see heat.
    expect(backfill).not.toHaveProperty("must_see");
  });
...
describe("F91 skeleton dinner + single-attraction split (TC-M23-91-04)", () => {
  ...
  it("should_require_verbatim_pool_stop_names_in_prompt", () => {
...
describe("buildSkeletonUserMessage traveler prefs (agent-itinerary-102)", () => {
  it("should_include_season_rule_and_display_names_when_cn_family_kids", () => {
    ...
    expect(msg).toMatch(/7岁儿童/);
    expect(msg).toMatch(/偏好|preference/i);
  ...
  it("should_not_require_dropping_seasonal_pool_names_from_candidates", () => {
