...
| User test cases (ChatBox MCP) | [`8.user-test-cases.md`](./8.user-test-cases.md) |
| Technical design | [`5.agent-design.md`](./5.agent-design.md) |

**Status:** MVP-1 and **MVP-2 accepted** 2026-08-19 (operator confirmed usable). User stories plus Given-When-Then in [`2.agent-ac.md`](./2.agent-ac.md).
...
**Build order inside a slice:** one user story to DoD at a time ([`5.agent-design.md`](./5.agent-design.md) §16). Suggested MVP-1 order: **14 → 15 → 16 → 19 → 18 → 17 → 12 → 11 → 6 → 5 → 1 → 3 → 7 → 4 → 13**. Suggested MVP-2 order: **2 → 9 → 8 → 10**.
...
| 6 | agent | Map vendor selection | `places-agent-map-vendors` | Callers pass which **map vendors** to query (`providers[]`); agent validates credentials and capabilities; no silent vendor swap. `GOOGLE_MAPS` uses direct REST then Cloudflare Worker MCP (ADR-017) | [2.agent-ac.md](./2.agent-ac.md) | **1** |
...
| 8 | agent | Tripadvisor enrichment | `places-agent-tripadvisor-enrich` | Optional Tripadvisor ratings/content by name + location match; never pass Google `place_id` as an id | [2.agent-ac.md](./2.agent-ac.md) | **2** |
...
| 10 | agent | Natural-language place chat | `places-agent-nl-chat` | Traveler asks in natural language (optional file/image upload); agent runs a tool loop on server Quanzil | [2.agent-ac.md](./2.agent-ac.md) | **2** |
...
### User Story 5 — Timed day plan with origin (detail timed)
...
### User Story 6 — Weather shapes the timed plan
...
### User Story 7 — Meals in the timed plan (Story B)
...
### User Story 8 — Live direction legs (Story C)
