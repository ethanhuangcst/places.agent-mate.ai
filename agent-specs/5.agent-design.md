...
| Same tool functions on HTTP and MCP | Forked MCP-only business logic |
...
| Path | Auth | Role |
| --- | --- | --- |
| `/` `/login` `/reset-password` `/set-password` `/instructions` | Public (session optional) | Operator HTML |
...
Six public tools is above the 3–5 chatbot default because this is a **dual-transport contract for three callers**. Do not add more. Parallel AMAP+Google inside one tool is **adapter fan-out**. Tripadvisor enrich and Open-Meteo are **server-side**.
...
| Adapter | Id | Notes |
| --- | --- | --- |
| AMAP Web 服务 | `AMAP` | `lng,lat`; GCJ-02 in CN/HK/MO/TW; convert WGS-84 via `/v3/assistant/coordinate/convert`; dining `types=050000`; **no** `weatherInfo` |
| Google direct REST | `GOOGLE_MAPS` | Places New / Geocoding / Routes; WGS-84; `languageCode` from locale map |
| Google Worker MCP | same `GOOGLE_MAPS` | After direct egress failure. `GMAPS_MCP_*`. `tools/list` first. Never send `GOOGLE_MAPS_API_KEY` to the Worker |
| Tripadvisor Terra | `TRIPADVISOR` | `lat`+`lon`, `unit=KM`; name+location only (ADR-007) |
| Open-Meteo | `OPEN_METEO` | **Not** in `providers[]`. Keep `weather_code` + numbers; localize `weather.wmo.{code}` |
...
| `/set-password` | 15 US5 | `04-set-password.html` | Session or invite/reset token |
| `/instructions` | 18 | `05` / `11` | Public or session; same body; literal `places-agent` |
| `/admin` | 16 | — | Session; **redirect** → `/admin/api-keys` |
| `/admin/api-keys` | 16 + 17 | `06-keys.html` | Session; **landing after login** |
...
**i18n:** custom catalogs `messages/{EN,CN,HK,TW}.json` + `t(locale, key, vars)`. **Do not add `next-intl`.** Missing key → `EN` → key. Seed from `ui-mockup/assets/i18n.js` (drop gallery keys). HK and TW must differ. Emails reuse the same files. `POST /api/admin/locale` then `router.refresh()`. `html lang`: `en` / `zh-CN` / `zh-HK` / `zh-TW`.

**Data:** React Query → same-origin `/api/admin/*` with `credentials: "include"`. Zustand not required. RHF + Zod on forms.
...
Three product slices **by agent capability** — see [`1.agent-stories.md`](./1.agent-stories.md) MVP plan. Finish **MVP-1** (including **all admin UI 14–19**) before MVP-2.
...
**MVP-2 — Search places + plan itinerary:** 2 `search_places` → 9 `plan_itinerary` + Open-Meteo (`weather.wmo.*`).

**MVP-3 — Enrich + place chat:** 8 Tripadvisor enrich → 10 NL chat loop (reuses the tool core).  
